 ...
 * DayInfos entities. Transaction control of the save(), update() and delete()
...
public class DayInfosDAO extends HibernateDaoSupport implements IComplexDAO<DayInfos, DayInfosId> {
	...
	public DayInfos findById(trimatrix.db.DayInfosId id) {
		log.debug("getting DayInfos instance with id: " + id);
		try {
			DayInfos instance = (DayInfos) getHibernateTemplate().get("trimatrix.db.DayInfos", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<DayInfos> findByProperty(String propertyName, Object value) {
		log.debug("finding DayInfos instance with property: " + propertyName + ", value: " + value);
		try {
			String queryString = "from DayInfos as model where model." + propertyName + "= ?";
	...
	public DayInfos merge(DayInfos detachedInstance) {
		...
		try {
			DayInfos result = (DayInfos) getHibernateTemplate().merge(detachedInstance);
	...
	public static IComplexDAO<DayInfos, DayInfosId> getFromApplicationContext(ApplicationContext ctx) {
		return (IComplexDAO<DayInfos, DayInfosId>) ctx.getBean("DayInfosDAO");
	}
}